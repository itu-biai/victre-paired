...
correlation (primary), scale-matched PSNR, SSIM, RMSE, and mass SDNR.
...
summ = {}
for rg in ["ideal", "real"] + (["noisy"] if args.noisy_regime else []):
...
if args.noisy_regime and "noisy" in summ:
...
for density, (fp, b) in picks.items():
    try:
        ...
        z = int(masses[0, 0]) if len(masses) else G.A(gt).shape[0] // 2
...
# =============================================================================
manifest = dict(
    dataset=DATA, split=SPLIT, n_patients=INV["n"], methods=MORDER,
    regimes=REGIMES, inversion=INV,
    files=dict(
...
P(f"    FBP best for {INV['fbp_best']}/{INV['n']} patients, "
  f"gap negative for {INV['fbp_neg_gap']}/{INV['n']}")
